class GradeScale {
  ...
  getFrenchScale(fromGrade, toGrade) {
    return this.getTable().map(grade => grade.french);
  }

  /**
   * Returns the Yosemite Decimal System (YDS) scale as an array.
   */
  getYdsScale(fromGrade, toGrade) {
    return this.getTable().map(grade => grade.yds);
  }

  /**
   * Returns the Vermin (V) scale as an array.
   */
  getVScale(fromGrade, toGrade) {
    return this.getTable().map(grade => grade.v);
  }
  ...
  getFontScale(fromGrade, toGrade) {
    return this.getTable().map(grade => grade.font);
  }
